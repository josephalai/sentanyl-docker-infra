#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════════════
# SENTANYL — End-to-End MailHog Full Sequence
...
#   • Click link in A        → kill A, jump immediately to C-Sc1
#   • No click in A (all 3)  → auto-advance sequentially to B-Sc1
#   • Click link in B        → kill B, jump immediately to C-Sc1
#   • No click in B (all 3)  → skip to next STORYLINE (B-Sc3 has skip_storyline_on_expiry=true)
#   • Click link in C        → kill C, jump immediately to next STORYLINE
#   • No click in C (all 3)  → auto-advance sequentially to D-Sc1
#   • Click link in D        → kill D, jump immediately to next STORYLINE
...
#   │   ├── Acts 7–9  : Enactment C scenes 1-2-3  (Soft Sell, click→next storyline)
#   │   └── Acts 10-12: Enactment D scenes 1-2-3  (Hard Sell, click→next storyline)
...
#   3.  bash scripts/e2e-mailhog-full-sequence.sh
#   4.  Open MailHog at http://localhost:8025 — first email arrives in seconds
#   5.  Click links to navigate; or wait 1 min for the next scene to auto-arrive
#
# ENV OVERRIDES:
...
<body style=\"font-family:Arial,sans-serif;max-width:600px;margin:32px auto;background:${bg_color};padding:28px;border-radius:12px\">
  ...
  <p style=\"color:${text_color};font-size:12px\">
    (Scene ${sc} of 3 — clicking BUY NOW marks your interest and routes you forward.)
  </p>"
                ;;
                D)
                    body="${body}
...
# ═══════════════════════════════════════════════════════════════════════════════
hdr "PHASE 4 — ADD TRIGGERS TO ALL 36 ENACTMENTS"
# ═══════════════════════════════════════════════════════════════════════════════
#
# Each enactment gets one OnWebhook/OnClick trigger.
#
# Enactment types A and B: clicking the "More Info" link jumps straight to
#   Enactment C Scene 1 of the SAME storyline (next_enactment with only _id
#   set — NextEnactment() only reads enactment.Id).
#
# Enactment types C and D: clicking "BUY NOW" marks the story as complete
#   (mark_complete: true).  AdvanceToNextEnactment will still walk through
#   any remaining C/D scenes (each 1 min apart) before advancing to the
#   next storyline once all 12 acts in the storyline are exhausted.
#
# ALL triggers carry when.wait_until = 1 minute, which is what the background
# scheduler uses to fire auto-advancement when no click arrives.
...
for sl in 1 2 3; do
    ...
    for ti in 0 1 2 3; do
        ...
        for sc in 1 2 3; do
            ...
            case "$et" in
                A|B)
                    # Click → jump to Soft Sell (EC-Sc1) of this storyline.
                    # next_enactment._id is the MongoDB ObjectId of EC-Sc1 for this
                    # storyline — NextEnactment() resolves the enactment by _id.
                    ACTION_JSON="{
                        \"action_name\": \"Jump to Soft Sell (EC-Sc1)\",
                    ...
                    ;;
                C|D)
                    # Click "BUY NOW" → immediately jump to the next storyline.
                    # advance_to_next_storyline=true causes ExecuteAction to call
                    # God.AdvanceToNextStoryline(), bypassing any remaining C/D scenes.
                    ACTION_JSON="{
                        \"action_name\": \"BUY NOW — Advance to Next Storyline\",
...
echo ""
echo -e "${BLD}${GRN}════════════════════════════════════════════════════════════════${RST}"
echo -e "${BLD}${GRN}  ✅  SETUP COMPLETE — Story A is running!${RST}"
echo -e "${BLD}${GRN}════════════════════════════════════════════════════════════════${RST}"
echo ""
...
echo "  Full no-click path: 36 scenes × 1 min ≈ 36+ min to complete all 3 workshops (plus scheduling delays)."
echo ""
echo -e "${BLD}${YLW}WHAT CLICKING DOES:${RST}"
echo "  • Click 'Get More Info' in Enactment A or B → kills current enactment, jumps to C-Sc1"
echo "  • Click 'BUY NOW'       in Enactment C or D → kills current enactment, jumps to NEXT STORYLINE"
...
echo "  A clicked→C→D→NextSL:   SL1: EA1→(click)→EC1→EC2→EC3→ED1→ED2→ED3→[SL2]"
...
echo ""
echo -e "${BLD}${CYN}════════════════════════════════════════════════════════════════${RST}"
echo -e "${BLD}${CYN}  Happy testing! Watch the emails flow in MailHog. 🚀${RST}"
echo -e "${BLD}${CYN}════════════════════════════════════════════════════════════════${RST}"
echo ""
